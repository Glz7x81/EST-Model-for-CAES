...
%% Heat Exchanger
figure;
plot(tout/unit("day"), EHeatExchanger/unit("J"));
xlim([0 tout(end)/unit("day")]);
grid on;
title('Energy Stored in Heat Exchanger');
xlabel('Time [day]');
ylabel('Energy [J]');
...
EtoHeatExchanger = sum(EHeatExchanger);
EtoStorage = EtoInjection + EtoHeatExchanger;

EfromHeatExchanger = sum(EHeatExchanger);
EfromStorage = EfromExtraction + EfromHeatExchanger;
